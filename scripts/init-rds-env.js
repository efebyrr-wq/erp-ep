...
/**
 * RDS database initialization using environment variables
 * Usage: 
 *   DB_PASSWORD=yourpassword node scripts/init-rds-env.js
 *   Or: DB_HOST=... DB_PASSWORD=... node scripts/init-rds-env.js
 */
...
async function main() {
  const endpoint = process.env.DB_HOST || 'erp-2025-db.cb4eq8qaqm7h.eu-north-1.rds.amazonaws.com';
  const password = process.env.DB_PASSWORD;
  const username = process.env.DB_USERNAME || 'postgres';
  const databaseName = process.env.DB_NAME || 'erp_2025';
  const port = parseInt(process.env.DB_PORT || '5432');

  if (!password) {
    console.error('❌ Error: DB_PASSWORD environment variable is required!');
    console.error('\nUsage:');
    console.error('  DB_PASSWORD=yourpassword node scripts/init-rds-env.js');
    console.error('\nOr with all options:');
    console.error('  DB_HOST=endpoint DB_PASSWORD=pass DB_USERNAME=user DB_NAME=dbname node scripts/init-rds-env.js');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('⚠️  Remember to set DB_PASSWORD in your environment variables (not shown above for security)\n');
